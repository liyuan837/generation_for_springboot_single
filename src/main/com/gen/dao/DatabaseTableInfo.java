...
public class DatabaseTableInfo {
	...
	private DatabaseTableInfo(String tableName) throws SQLException {
		...
		// 初始化PO类
		po = this.createBeanFile(className + "Po", Config.poPackage + "." + className.toLowerCase(),  columnPropertiesList, null, 0);

		// 初始化condition类
		condition = this.createQueryBean(className + "Condition", Config.conditionPackage + "." + className.toLowerCase(), false);

		// 初始化queryForm类
		queryForm = this.createQueryBean(className + "QueryForm", Config.formPackage + "." + className.toLowerCase(), true);

		// 初始化createForm
		createForm = this.createInsertFrom(className + "CreateForm", Config.formPackage + "." + className.toLowerCase());

		// 初始化updateForm
		updateForm = this.createUpdateFrom(className + "UpdateForm", Config.formPackage + "." + className.toLowerCase());

		// 初始化deleteForm
		deleteForm = this.createDeleteFrom(className + "DeleteForm", Config.formPackage + "." + className.toLowerCase());

		// 初始化vo
		vo = this.createBeanFile(className + "Vo", Config.voPackage + "." + className.toLowerCase(), columnPropertiesList, null, 3);
     ...
     * @return
     */
	private BeanFile createBeanFile(String className, String packageName,List<ColumnProperties> propertiesList, List<String> defaultImportList, int beanType) {
		...
		beanFile.setImplementsClassName("Serializable");
     ...
     * @return
     */
	private BeanFile createQueryBean(String className, String packageName, boolean isForm) {
		...
		if (isForm) {
			...
			defaultImportList.add(Config.domainPackage+".condition.BaseCondition");
		}

		BeanFile beanFile = this.createBeanFile(className, packageName,  conditionPropertiesList, defaultImportList, isForm ? 1 : 0);
		if (isForm) {
	 ...
	 * @return
	 */
	private BeanFile createInsertFrom(String className, String packageName) {
		ColumnProperties primaryKey = this.getPrimaryKey();
		...
		BeanFile beanFile = this.createBeanFile(className, packageName, formPropertiesList, null, 1);
		return beanFile;
	}
	 ...
	 * @return
	 */
	private BeanFile createUpdateFrom(String className, String packageName) {
		...
		BeanFile beanFile = this.createBeanFile(className, packageName,  formPropertiesList, null, 2);
		return beanFile;
	}
	 ...
	 * @return
	 */
	private BeanFile createDeleteFrom(String className, String packageName) {
		List<ColumnProperties> formPropertiesList = new ArrayList<>();
		for (ColumnProperties columnProperties : po.getPropertiesList()) {
